...
func connect_building_part(area, area2):
	if area2.is_in_group("building_colliders"):
		...
		print("wall connected to %s" % area.name)
		if area2.is_in_group("collider_wall"):
			area2.connected_roof = area
			print("roof connected to %s" % area2.name)
