...
const TAB_SCENE = preload("res://scenes/interface/crafting/craft_menu_tab.tscn")
const CRAFT_ITEM_SCENE = preload("res://scenes/interface/crafting/craft_item.tscn")
...
@export var tabs_resource : CraftMenuListData
...
func set_craft_menu_tabs(craft_menu_tab : CraftMenuListData) -> void:
	for tab_data : CraftMenuTabData in craft_menu_tab.list:
		if tab_data:
			var tab := TAB_SCENE.instantiate()
			tab_container.add_child(tab)
			tab.set_tab_data(tab_data)
			tab.signal_tab_clicked.connect(_on_tab_clicked)
			set_craftable_items(tab_data.slots_in_tab)
...
func set_craftable_items(slots_in_tab : Array[InSlotData]) -> void:
	for slot_data : InSlotData in slots_in_tab:
		if not slot_data.item: return push_error("CRAFT MENU: No item data in slot data")
		var craft_item := CRAFT_ITEM_SCENE.instantiate()
		match slot_data.item.item_type:
			EnumData.ItemType.FIREARM:
				item_weapon_container.add_child(craft_item)
			EnumData.ItemType.CONSUMABLE:
				item_heal_container.add_child(craft_item)
			EnumData.ItemType.BUILDING:
				item_buildings_container.add_child(craft_item)
			EnumData.ItemType.TOOL:
...
func _on_craft_button_pressed(slot_data: InSlotData) -> void:
	...
	var player_inventory_slots : Array[InSlotData] = player.player_inventory.slots_data
	var player_quick_slots : Array[InSlotData] = player.player_quick_slot.slots_data
