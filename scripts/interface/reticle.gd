...
func adjust_reticle_lines():
	...
	if item_script.timer.is_stopped() == false:
		if crosshair_range < 1:
			shooting_multiplier_value = crosshair_range * -player.current_weapon_spread_data.shooting_state_multiplier
		else:
			shooting_multiplier_value = crosshair_range * player.current_weapon_spread_data.shooting_state_multiplier
	else:
		shooting_multiplier_value = 0
	spread_factors = crosshair_range + (player_velocity + shooting_multiplier_value) * player.current_weapon_spread_data.all_factors_multiplier
